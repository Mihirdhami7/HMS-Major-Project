...
import { getUserProfile, getPatientByEmail, getHospitalMedicines, savePrescription as savePrescriptionAPI } from "../../services/userService";
...
function Prescription() {
    ...
    useEffect(() => {
        const initializeData = async () => {
            ...
            if (profileResult.success) {
                setDoctorInfo(profileResult.data);
                setHospitalName(profileResult.data.hospitalName || "Zydus");
                
                // Fetch medicines after getting hospital name
                await fetchAvailableMedicines(profileResult.data.hospitalName, token);
            }

            // Fetch patient details if available
            if (appointmentData?.patientEmail) {
                await fetchPatientDetails(appointmentData.patientEmail, token);
            }
        };
    ...
    const fetchPatientDetails = async (patientEmail, token) => {
        try {
            setLoading(true);
            setError(null);
            
            const result = await getPatientByEmail(patientEmail, token);
    ...
    // Fetch medicines from database based on hospital
    const fetchAvailableMedicines = async (hospital, token) => {
        try {
            setLoading(true);
            const result = await getHospitalMedicines(hospital, token);
            
            if (result.success) {
                const medicines = result.data.map(med => ({
                    id: med._id,
                ...
                throw new Error(result.error || "Failed to load medicines");
            }
        } catch (err) {
    ...
    const savePrescription = async () => {
        try {
            ...
            if (!token || !doctorInfo) {
                setError("Session expired or doctor information missing. Please login again.");
                return false;
            }
            ...
            const prescriptionData = {
                ...
                doctorEmail: doctorInfo.email || "doctor@example.com",
            ...
            // Send the data to the backend using API service
            const result = await savePrescriptionAPI(prescriptionData, token);
            ...
            if (result.success) {
                ...
                throw new Error(result.error || "Failed to send prescription data");
            }
        } catch (err) {
