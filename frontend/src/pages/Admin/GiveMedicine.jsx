...
import { getHospitalPrescriptions, getHospitalMedicines, getUserProfile } from '../../services/userService';
...
const Givemedicine = () => {
    ...
    useEffect(() => {
        const initData = async () => {
            ...
            if (userResult.success) {
                ...
                fetchPrescriptions(hName, token);
    ...
    const handleRefreshClick = () => {
        const token = getToken();
        if (token) {
            fetchPrescriptions(hospitalName, token);
        }
    };
    ...
    const fetchPrescriptions = async (hName, token) => {
        try {
            setLoading(true);
            const result = await getHospitalPrescriptions(hName, token);
            if (result.success) {
    ...
    const fetchAvailableMedicines = async (medicineList, hName, token) => {
        try {
            const result = await getHospitalMedicines(hName, token);
            if (result.success) {
                setAvailableMedicines(Array.isArray(result.data) ? result.data : []);
            } else {
    ...
    const handlePrescriptionClick = async (prescription) => {
        ...
        const token = getToken();
        if (prescription.medicines?.length > 0 && token) {
            await fetchAvailableMedicines(prescription.medicines, hospitalName, token);
        }
    };
